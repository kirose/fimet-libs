...
public class StressIteratorFolder implements Iterator<Stress> {
	...
	public StressIteratorFolder(File folder) {
	...
	private static Stress parseStress(Path entry) {
		if (isStressFile(entry)) {
			try {
				Stress useCase = (Stress)StressUtils.fromFile(entry.toFile());
				return useCase;
			} catch (Exception e) {
				FimetLogger.error(StressIteratorFolder.class,"Invalid stress "+(entry.getFileName()),e);
				return null;
			}
		}
		return null;
