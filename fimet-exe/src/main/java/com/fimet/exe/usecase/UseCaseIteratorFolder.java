...
public class UseCaseIteratorFolder implements Iterator<UseCase> {
	...
	public UseCaseIteratorFolder(File folder) {
	...
	private static UseCase parseUseCase(Path entry) {
		if (isUseCaseFile(entry)) {
			try {
				...
				FimetLogger.error(UseCaseIteratorFolder.class,"Invalid use case "+(entry.getFileName()),e);
				return null;
			}
		}
		return null;
