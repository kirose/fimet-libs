...
public class Range {
	...
	public static void main(String[] args) {
		List<String> vals = generateRange(5L, 40L, 6L, '.', "0-9", "xx", "ZZ");
		for (String v : vals) {
			System.out.println(v);
		}
	}
	private static List<String> generateRange(Long start, Long end, Long length, char pad, String charset, String preappend, String append) {
		...
		long maxPadLn = range / charset.length() + ((range % charset.length()) >1?1:0);
		if (maxPadLn + lnp + lna > length) {
			end = (length - lnp - lna)*charset.length();
		}
	...
	private static String parseCharset(String charset) {
