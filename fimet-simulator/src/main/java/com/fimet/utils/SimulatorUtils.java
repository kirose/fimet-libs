...
import com.fimet.dao.SimulatorMessageXmlDAO;
...
public final class SimulatorUtils {
	...
	public static long hashCodeExtensions(List<JSimulator> simulators) {
		final int prime = 31;
		long result = 1;
	...
	public static ISimulatorExtension newSimulatorExtension(List<JSimulator> simulators) {
		Class<?> clazz = null;
		try {
			clazz = getClassValidator(simulators);
		} catch (Exception e) {
			FimetLogger.error(SimulatorUtils.class, "Error compiling simulator extension, will be use default extension", e);
			return NullSimulatorExtension.INSTANCE;
		}
		try {
			return (ISimulatorExtension)clazz.newInstance();
		} catch (Exception e) {
			FimetLogger.error(SimulatorUtils.class, "Error instantianting validator class "+clazz.getName()+", will be use default extension", e);
			return NullSimulatorExtension.INSTANCE;
		}
	}

	public static Class<?> getClassValidator(List<JSimulator> simulators) {
	...
	private static void createValidation(StringBuilder s, String name, String v) {
	...
	public static Class<ISimulatorModel> installClassSimulatorModel(IESimulatorModel simulator) {
		...
		Class<?> compile = Manager.get(ICompilerManager.class).compileAndLoad(classModel.getClassName(), classModel.getSource());
		return (Class<ISimulatorModel>)compile;		
	}
	@SuppressWarnings("unchecked")
	public static Class<ISimulatorModel> reinstallClassSimulatorModel(IESimulatorModel simulator) {
		ClassModel classModel = classModelFromSimulatorModel(simulator);
		Class<?> compile = Manager.get(ICompilerManager.class).compileAndReload(classModel.getClassName(), classModel.getSource());
		return (Class<ISimulatorModel>)compile;		
	}
	public static JSimulator toJSimulator(ISimulator s) {
		if (s!=null) {
	...
	public static JSimulator copy(ISimulator from, JSimulator to) {
	...
	public static JSimulator copy(IESimulator from, JSimulator to) {
		to.setName(from.getName());
	...
	public static String createValidationSource(String packageName, String simpleClassName, String validation) {
		...
		.append("import static ").append(packageAssertions).append("Assertions.*").append(";\n")
		.append("/**\n")
		.append("* FIMET\n")
	...
	private static String extractBlock(String validation, boolean fail) {
	...
	public static String classSourceParseName(String source) {
	...
	public static String classSourceParseAssertion(String source) {
	...
	private static String parseNameBlock(String source){
	...
	@SuppressWarnings("deprecation")
	private static String parseAssertionBlock(String source){
	...
	@SuppressWarnings("deprecation")
	private static String removeBlanks(String block) {
		int ln = block.length();
		int i = 0;
	...
	public static int hasCode(IESimulator s) {
		final int prime = 31;
		int result = 1;
	...
	public static boolean equals(IESimulator s1, IESimulator s2) {
		if (s1 == s2)
			return true;
		if (s1 == null)
			return false;
	...
	public static boolean equals(ISimulator s1, ISimulator s2) {
		if (s1 == s2)
			return true;
		if (s1 == null)
			return false;
	...
	public static boolean equals(ISimulator s1, IESimulator s2) {
		if (s1 == s2)
			return true;
		if (s1 == null)
			return false;
