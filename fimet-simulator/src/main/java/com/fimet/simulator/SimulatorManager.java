...
public class SimulatorManager implements ISimulatorManager {
	private static final NullSimulator NONE = new NullSimulator();
	...
	public  ISimulator getSimulatorIssuer(Integer id) {
		...
		issuers.put(id, new SimulatorImpl(entity.getId(), entity.getName()));
		return issuers.get(id);
	}
	public  ISimulator getSimulatorAcquirer(Integer id) {
		...
		Simulator entity = getEntity(id);
		acquirers.put(id, entity != null ? new SimulatorImpl(entity.getId(), entity.getName()) : NONE);
		return acquirers.get(id);
	}
	...
	public ISimulator getSimulator(Integer id) {
		...
		Simulator entity = getEntity(id);
		if (entity == null) return NullSimulator.INSTANCE;
		if (entity.getType() == Simulator.ISSUER) {
			issuers.put(id, new SimulatorImpl(entity.getId(), entity.getName()));
			return issuers.get(id);
		} else {
			acquirers.put(id, new SimulatorImpl(entity.getId(), entity.getName()));
			return acquirers.get(id);
		}
	}
	@Override
	public ISimulator getSimulator(Simulator entity) {
		...
		if (entity.getType() == Simulator.ISSUER) {
			issuers.put(entity.getId(), new SimulatorImpl(entity.getId(), entity.getName()));
			return issuers.get(entity.getId());
		} else {
			acquirers.put(entity.getId(), entity != null ? new SimulatorImpl(entity.getId(), entity.getName()) : NONE);
			return acquirers.get(entity.getId());
		}
	}
	@Override
	public ISimulator getSimulator(String name) {
		for (Map.Entry<Integer, ISimulator> e : issuers.entrySet()) {
			if (e.getValue().getName().equals(name)) {
				return e.getValue();
			}
		}
		for (Map.Entry<Integer, ISimulator> e : acquirers.entrySet()) {
			if (e.getValue().getName().equals(name)) {
				return e.getValue();
			}
		}
		Simulator entity = getEntity(name);
		if (entity == null) return NullSimulator.INSTANCE;
		if (entity.getType() == Simulator.ISSUER) {
			issuers.put(entity.getId(), new SimulatorImpl(entity.getId(), entity.getName()));
			return issuers.get(entity.getId());
		} else {
			acquirers.put(entity.getId(), new SimulatorImpl(entity.getId(), entity.getName()));
			return acquirers.get(entity.getId());
		}
	}
}
