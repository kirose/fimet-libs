...
import com.fimet.dao.ParserXmlDAO;
...
public class ParserManager extends AbstractManager implements IParserManager {
	...
	IParserDAO dao = Manager.get(IParserDAO.class, ParserXmlDAO.class);
	...
	public IParser getParser(String name) {
		if (mapNameParsers.containsKey(name)) {
			...
			if (entity!=null) {
				...
				Manager.get(IEventManager.class).fireEvent(ParserEvent.PARSER_LOADED, this, parser);
				return parser;
			} else {
				return null;
			}
	...
	public List<IParser> getParsers() {
		List<IEParser> entities = dao.findAll();
		if (entities != null) {
	...
	public IParser remove(String parser) {
		if (mapNameParsers.containsKey(parser)) {
			...
			Manager.get(IEventManager.class).fireEvent(ParserEvent.PARSER_REMOVED, this, p);
			return p;
		} else {
			return null;
		}
