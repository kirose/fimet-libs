...
public class MessageUtils {
		...
		if (ln <= 6) {//BIN
			...
			if (oldLn > ln) {
				...
				if (newLn<38) {
					...
					FimetLogger.warning(MessageUtils.class,"Replace "+pan+" in track 2 "+track2+" los info");
					return (pan + track2.substring(oldLn)).substring(0,38);
				}
			} else {
				return pan + track2.substring(ln); 
			}
			//FimetLogger.warning(MessageUtils.class,"Bad track 2 for "+track2);
		}
	}
	...
	public static long longHashCode(IMessage message) {
		if (message != null) {
