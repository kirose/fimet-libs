...
import com.fimet.dao.FieldGroupXmlDAO;
...
public class FieldGroupManager extends AbstractManager implements IFieldGroupManager {
	...
	private IFieldGroupDAO dao = Manager.get(IFieldGroupDAO.class, FieldGroupXmlDAO.class);
	...
	private void loadGroups() {
		List<IEFieldGroup> allGroups = dao.findAll();
	...
	public IFieldGroup remove(String name) {
		if (mapNameGroup.containsKey(name)) {
			...
			Manager.get(IEventManager.class).fireEvent(ParserEvent.FIELDGROUP_REMOVED, this, group);
			return group;
		} else {
			return null;
		}
	...
	public IFieldGroup reload(String name) {
		if (mapNameGroup.containsKey(name)) {
			...
			group.reload();
			Manager.get(IEventManager.class).fireEvent(ParserEvent.FIELDGROUP_REMOVED, this, group);
			Manager.get(IEventManager.class).fireEvent(ParserEvent.FIELDGROUP_LOADED, this, group);
			return group;
		} else {
			...
			if (group != null) {
				...
				FimetLogger.warning(getClass(), "Unkown Field Group "+name);
				return null;
			}
		}
	}
