...
public class AbstractDAO<E, T> {
	private IThreadManager threadManager = Manager.get(IThreadManager.class);
	private Dao<E,T> dao;
	...
	public AbstractDAO() {
		this(Manager.get(ISQLiteManager.class).getFimetConnection());
	}
	...
	private void createDAO() {
		if (dao == null) {
			...
			try {
				...
				FimetLogger.error("Cannot create dao for class: " + classE.getName(), e);
			}
		}
	}
	...
	public void insertAsync(E entity) {
		threadManager.execute(()->{
			try {
				//System.out.println(new Timestamp(System.currentTimeMillis())+"-START-INSERT-"+"-"+entity);
				synchronized (AbstractDAO.this) {
					AbstractDAO.this.getDAO().create(entity);	
				}
				//System.out.println(new Timestamp(System.currentTimeMillis())+"-END-INSERT-"+"-"+entity);
			} catch (SQLException e) {
				FimetLogger.error(getClass(), "Persistence exception", e);
			}
		});
	}
	public void insertOrUpdateAsync(E entity) {
		threadManager.execute(()->{
		try {
			synchronized (AbstractDAO.this) {
