...
import com.fimet.iso8583.parser.Field;
import com.fimet.iso8583.parser.IFieldParser;
import com.fimet.iso8583.parser.MessageLayout;

public class MessageLayoutAdapter extends TypeAdapter<MessageLayout>{
	...
	@Override
	public MessageLayout read(JsonReader in) throws IOException {
	...
	private void readFields(JsonReader in, MessageLayout message) throws IOException{
	...
	private void readField(JsonReader in, String idParent, MessageLayout message) throws IOException {
		if (!in.hasNext()) {
			return;
		}
	...
	private void formatParent(MessageLayout message, String idField) {
		IFieldParser fieldParser = fieldParserManager.getFieldParser(message.getParser().getIdGroup(), idField);
		if (fieldParser == null) {
			FimetLogger.warning(MessageLayoutAdapter.class, message.getParser()+", Unknow field "+idField);
		} else {
			try {
				...
				FimetLogger.warning(MessageLayoutAdapter.class, "Cannot format field "+idField+": "+e.getMessage(),e);
			}
		}
	}
	@Override
	public void write(JsonWriter out, MessageLayout msg) throws IOException {
		 out.beginObject()
	...
	private void writeFields(JsonWriter out, MessageLayout message) throws IOException{
 		out.name("fields");
 		out.beginObject();
