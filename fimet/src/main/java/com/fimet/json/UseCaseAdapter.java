...
public class UseCaseAdapter extends TypeAdapter<IUseCase>{
	protected final TypeAdapter<IUseCase> delegate;
	private TypeAdapter<IMessage> messageAdapter;
	private TypeAdapter<UseCaseJson> useCaseJsonAdapter;
	public UseCaseAdapter(TypeAdapter<IUseCase> delegate) {
		if (delegate == null) {
			throw new NullPointerException("TypeAdapter delegate is null");
		}
		this.delegate = delegate;
		this.messageAdapter = JsonAdapterFactory.GSON.getAdapter(IMessage.class);
		this.useCaseJsonAdapter = JsonAdapterFactory.GSON.getAdapter(UseCaseJson.class);
	}
	@Override
	public IUseCase read(JsonReader in) throws IOException {
		UseCaseJson json = useCaseJsonAdapter.read(in);
			...
			if (uc.getSimulators() != null && !uc.getSimulators().isEmpty()) {
				...
				for (ISimulator	s : uc.getSimulators()) {
					...
					if (s.getSocket() != null) {
						...
						out.name("server").value(socket.server());
