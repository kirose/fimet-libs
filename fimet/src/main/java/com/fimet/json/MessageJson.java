...
public class MessageJson {
	...
	public class MessageFields implements Cloneable {
		...
		int[] getBitmap() {
			...
			int i = 0;
			while((node = node.next)!=null) {
				bitmap[i++] = Integer.parseInt(node.id);
			}
			return bitmap;
		}
		/**
		...
		/**
		 * Find the field in the binary tree identified by idField
		 * Each idField has associated an short[] address
		 * @param idField
		 * @return
		 */
		Node find(String idField) {
			String[] address = getAddress(idField);
		 ...
		 * @param id
		 * @param leaf
		 * @return Node the node found or inserted
		 */
		Node getOrInsert(Node head, String idField, String id, boolean leaf) {
			Node node = head;
			while (node.next != null && !node.next.id.equals(id)) {
				node = node.next;
			}
			if (node.next != null) {
				if (node.next.id == id) {
		 ...
		 */
		Node insert(Node head, String idField, String id, byte[] value, boolean leaf) {
			Node node = head;
			while (node.next != null && !node.next.id.equals(id)) {
				node = node.next;
			}
		 ...
		 * @return
		 */
		Node find(Node head, String id) {
			Node node = head;
			while ((node = node.next) != null) if (node.id.equals(id)) return node;
			return null;
		}
		/**
		 ...
		 * @return
		 */
		Node remove(Node head, String id) {
			Node node = head;
			while (node.next != null && !node.next.id.equals(id)) {
				node = node.next;
			}
			if (node.next != null && node.next.id.equals(id)) {
				Node item = node.next; 
		 ...
		 */
		String[] getAddress(String idField) {
			if (idField == null) {
				throw new NullPointerException();
			} else {
				return idField.split("\\.");
			}
		}
		public MessageFields clone() throws CloneNotSupportedException {
			MessageFields clone = new MessageFields();
		...
		abstract class Node implements Cloneable {
			...
			byte[] bytes;
			Node() {}
			Node(String idField, String id, byte[] value) {
		...
		class Leaf extends Node {
			...
			Leaf(String idField, String id, byte[] value) {
		...
		class Parent extends Node {
			...
			Parent(String idField, String id, byte[] value) {
				super(idField, id, value);
				head = new Leaf();
			}
			public boolean hasChildren() {
